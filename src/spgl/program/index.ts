...
class Program {
  ...
  compile(vert: string, frag: string, prefix?: string ): boolean {
    ...
    this.fShaderStr  = prefix + frag;
    this.vShaderStr  = prefix + vert;
 ...
 */
function compileShader(gl: WebGLRenderingContext, shader: WebGLShader, code: string): boolean {
 ...
 */
function getUniformSetter(type: number, location: WebGLUniformLocation, gl: WebGLRenderingContext, context: CompilationContext) {
  switch (type) {
    case gl.FLOAT_MAT2:
    case gl.FLOAT_MAT3:
    case gl.FLOAT_MAT4:
